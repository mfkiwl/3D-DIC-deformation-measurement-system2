...
criteria = (cv.TERM_CRITERIA_EPS + cv.TERM_CRITERIA_MAX_ITER, CF_user.CAL_ITERATION_TIMES, CF_user.CAL_ACCURACY)

# 建構object矩陣
objp = np.zeros((CF_user.CAL_CHESSBOARD_SIZE[0] * CF_user.CAL_CHESSBOARD_SIZE[1], 3), np.float32)
objp[:,:2] = np.mgrid[0:CF_user.CAL_CHESSBOARD_SIZE[0],0:CF_user.CAL_CHESSBOARD_SIZE[1]].T.reshape(-1,2)

objp = objp * CF_user.CAL_SQUARE_SIZE # 填入棋盤方格邊長 (mm)
print('objp=',objp)
...
for imgLeft, imgRight in zip(image_Left, image_Right): # zip: 將2組array中對應的元素打包成一個位組，最後傳回一個列表。
    ...
    retL, cornersL = cv.findChessboardCorners(grayL, CF_user.CAL_CHESSBOARD_SIZE, None)
    ...
    if retL and retR == True:
        ...
        cv.drawChessboardCorners(imgL, CF_user.CAL_CHESSBOARD_SIZE, cornersL, retL)
        cv.imshow('img left', imgL)
        cv.drawChessboardCorners(imgR, CF_user.CAL_CHESSBOARD_SIZE, cornersR, retR)
        cv.imshow('img right', imgR)
        cv.waitKey(200) # wait for displaying images (ms)。
        
cv.destroyAllWindows()


### ===== Single Camera Calibraiton ===== ###
# cv.getOptimalNewCameraMatrix: 進一步調整內部參數矩陣
retL, CameraMatrixL, distL, rvecsL, tvecsL = cv.calibrateCamera(objpoints, imgpointsL, CF_user.CAL_IMAGE_RES, None, None)
# heightL, widthL, channelsL = imgL.shape
# newCameraMatrixL, roi_L = cv.getOptimalNewCameraMatrix(cameraMatrixL, distL, (widthL, heightL), 1, (widthL, heightL))
retR, CameraMatrixR, distR, rvecsR, tvecsR = cv.calibrateCamera(objpoints, imgpointsR, CF_user.CAL_IMAGE_RES, None, None)
...
cv_file = cv.FileStorage(CF.STEREP_MAP_PATH, cv.FILE_STORAGE_WRITE)
